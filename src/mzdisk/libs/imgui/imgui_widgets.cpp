...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    const float button_sz = ImTrunc(g.FontSize * 0.55f);
    const ImVec2 button_pos(ImMax(bb.Min.x, bb.Max.x - frame_padding.x - button_sz), bb.Min.y + frame_padding.y + ImTrunc((g.FontSize - button_sz) * 0.5f));
    ...
    {
        // Inline tab close button: menší, v rámečku, vždy viditelný
        ImGuiLastItemData last_item_backup = g.LastItemData;
        ImGuiWindow* window = g.CurrentWindow;

        const ImRect cbb(button_pos, button_pos + ImVec2(button_sz, button_sz));
        ImRect cbb_interact = cbb;

        if (!ItemAdd(cbb_interact, close_button_id))
        {
            g.LastItemData = last_item_backup;
        }
        else
        {
            bool btn_hovered, btn_held;
            bool btn_pressed = ButtonBehavior(cbb_interact, close_button_id, &btn_hovered, &btn_held);
            if (btn_pressed)
                close_button_pressed = true;

            // Rámeček vždy viditelný
            ImU32 border_col = GetColorU32(ImGuiCol_Border, 0.6f);
            ImU32 bg_col = (btn_held && btn_hovered) ? GetColorU32(ImGuiCol_ButtonActive)
                         : btn_hovered ? GetColorU32(ImGuiCol_ButtonHovered)
                         : GetColorU32(ImGuiCol_FrameBg, 0.5f);
            window->DrawList->AddRectFilled(cbb.Min, cbb.Max, bg_col, 2.0f);
            window->DrawList->AddRect(cbb.Min, cbb.Max, border_col, 2.0f);

            // Křížek
            const ImU32 cross_col = GetColorU32(ImGuiCol_Text, btn_hovered ? 1.0f : 0.7f);
            const ImVec2 cross_center = cbb.GetCenter() - ImVec2(0.5f, 0.5f);
            const float cross_extent = button_sz * 0.5f * 0.7071f - 1.0f;
